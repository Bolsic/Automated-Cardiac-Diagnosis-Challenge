...
class _UpBlock(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels, use_batch_norm=True):
        super().__init__()
        ...
        if use_batch_norm:
            upsample_layers.append(nn.BatchNorm2d(out_channels))
...
class UNet2D(nn.Module):
    """2D U-Net segmentation network for cardiac MR slices."""
    ...
    def __init__(
        ...
        self.decoder4 = _UpBlock(base_channels * 16, base_channels * 8, base_channels * 8, use_batch_norm)
        self.decoder3 = _UpBlock(base_channels * 8, base_channels * 4, base_channels * 4, use_batch_norm)
        self.decoder2 = _UpBlock(base_channels * 4, base_channels * 2, base_channels * 2, use_batch_norm)
        self.decoder1 = _UpBlock(base_channels * 2, base_channels, base_channels, use_batch_norm)
