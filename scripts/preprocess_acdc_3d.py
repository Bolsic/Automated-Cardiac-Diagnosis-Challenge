...
def resize_in_plane(volume, target_height, target_width, mode):
    # Input volumes are stored as D x H x W. Conv3d interpolation expects
    # N x C x D x H x W, so add a temporary batch and channel dimension.
    tensor = torch.from_numpy(volume).float().unsqueeze(0).unsqueeze(0)

    if mode == "nearest":
        resized = F.interpolate(
            tensor,
            size=(volume.shape[0], target_height, target_width),
            mode=mode,
        )
    else:
        resized = F.interpolate(
...
def preprocess_volume(source_path, output_path, target_depth, target_height, target_width):
    with h5py.File(source_path, "r") as source:
        image = source["image"][:].astype(np.float32)
        label = source["label"][:].astype(np.uint8) if "label" in source else None

    image = resize_in_plane(image, target_height, target_width, mode="trilinear")
    image = normalize_image(image)
    image = center_crop_or_pad(image, target_depth, target_height, target_width, pad_value=0).astype(np.float32)

    if label is not None:
        label = resize_in_plane(label, target_height, target_width, mode="nearest")
    ...
    with h5py.File(output_path, "w") as output:
        ...
        output.attrs["preprocessing"] = "in-plane resize, depth center pad/crop, zero-mean unit-variance image"


def parse_args():
    parser = argparse.ArgumentParser(description="Export ACDC preprocessed volumes for 3D U-Net training.")
    parser.add_argument("--input-dir", type=Path, default=Path("ACDC_preprocessed/ACDC_training_volumes"))
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/acdc_preprocessed_3d/ACDC_training_volumes"))
